...
export async function getServerSideProps(context: GetServerSidePropsContext): Promise<GetServerSidePropsResult<ServerProps>> {
    ...
    let project = (JSON.parse(readFileSync('data/projects.json', 'utf8').toString()) as Project[]).find(p => p.id == id)
    ...
    let markdown = file != undefined
        ? readFileSync('data/projects/' + file).toString()
        : null
